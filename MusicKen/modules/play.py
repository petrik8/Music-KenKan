...
async def ee(client, message):
    ...
    if stats:
        ...
        await message.reply("Obrolan belum dimulai")
...
async def hfmm(_, message):
    ...
    if len(message.command) != 2:
        await message.reply_text(
            "Berikan perintah `/musicplayer on` atau `/musicplayer off`"
        )
        return
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not message.chat.id in DISABLED_GROUPS:
            await lel.edit("Bot musik berhasil diaktifkan")
            return
        DISABLED_GROUPS.remove(message.chat.id)
        await lel.edit(
            f"Bot musik berhasil diaktifkan {message.chat.id}"
        )
        ...
        if message.chat.id in DISABLED_GROUPS:
            await lel.edit("Bot musik telah dinonaktifkan")
            return
        DISABLED_GROUPS.append(message.chat.id)
        await lel.edit(
            f"Bot musik telah dinonaktifkan {message.chat.id}"
        )
    else:
        await message.reply_text(
            "Berikan perintah `/musicplayer on` atau `/musicplayer off`"
...
async def p_cb(b, cb):
    ...
    if type_ == "playlist":
        ...
        if not queue:
            await cb.message.edit("Tidak ada lagu yang diputar")
        temp = []
        for t in queue:
            temp.append(t)
        now_playing = temp[0][0]
...
async def play(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                if message.chat.title.startswith("Channel Music: "):
                    await lel.edit(
                        f"Ingatlah untuk menambahkan {user.first_name} ke Channel Anda",
                    )
                    pass
                try:
                ...
                try:
                    ...
                    await lel.edit(
                        f"{user.first_name} berhasil bergabung dengan Group",
                    )
                    ...
                    # print(e)
                    await lel.edit(
                        f"`Flood Wait Error`\n{user.first_name} tidak dapat bergabung dengan grup. Pastikan pengguna tidak dibanned / tambahkan @{ASSISTANT_NAME} secara manual ke Grup.",
                    )
    try:
        ...
        return
    text_links=None
    await lel.edit("🔎 Lagu sedang dipersiapkan...")
    ...
    if audio:
        ...
        try:
            ...
            await lel.edit(
                "Lagu tidak ditemukan. Masukkan judul lagu yang lebih jelas."
            )
            print(str(e))
            return
        dlurl=url
        ...
        try:
          ...
          await lel.edit("Masukkan judul lagu untuk diputar**")
        # Looks like hell. Aren't it?? FUCK OFF
        try:
            toxxt = "Pilih lagu yang akan diputar:\n\n"
            j = 0
            useer=user_name
            ...
            await lel.edit("Terjadi kesalahan permintaan, mulai bermain langsung...")
            ...
            try:
                ...
                await lel.edit(
                "Lagu tidak ditemukan. Masukkan judul lagu dengan lebih jelas."
            )
                print(str(e))
                return
            dlurl=url
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        try:
            ...
            message.reply("Obrolan tidak aktif, saya tidak dapat bergabung")
            return
        await message.reply_photo(
            photo="final.png",
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    # print(e)
                    await lel.edit(
                        f"{user.first_name} tidak dapat bergabung dengan grup Anda karena banyaknya permintaan bergabung untuk userbot! Pastikan pengguna tidak dibanned / tambahkan @{ASSISTANT_NAME} secara manual ke Grup",
                    )
    try:
        await USER.get_chat(chid)
        # lmoa = await client.get_chat_member(chid,wew)
